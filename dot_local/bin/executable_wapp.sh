...
while true; do
  ...
  1 | "i" | "install")
    ...
    # --- NEW PROFILE SELECTION START ---
    echo
    echo "Select Chrome Profile for the WebApp:"
    echo "1) Personal (Profile Directory: Default)"
    echo "2) School (Profile Directory: Profile 1)"
    read -rp "Choose profile (1 or 2): " PROFILE_CHOICE

    PROFILE_DIR=""
    case "$PROFILE_CHOICE" in
    1) PROFILE_DIR="Default" ;;
    2) PROFILE_DIR="Profile 1" ;;
    *)
      echo "Invalid profile choice. Defaulting to Personal (Default)."
      PROFILE_DIR="Default"
      ;;
    esac

    # Construct the final execution command
    EXEC_COMMAND="$CHROME_BIN --profile-directory=\"$PROFILE_DIR\" --app=\"$URL\""
    # --- NEW PROFILE SELECTION END ---
    ...
    echo "Creating desktop entry ..."
    {
      ...
      echo "Name=$NAME"
      # Use the constructed EXEC_COMMAND
